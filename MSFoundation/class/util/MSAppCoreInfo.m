...
@implementation MSAppCoreInfo

SB_ARC_SINGLETON_IMPLEMENT(MSAppCoreInfo);
...
- (id)init {
    ...
    //是否越狱
    self.isJailBreak = [MSAppCoreInfo isJailed];
    
    self.clientSign = [MSFileManager stringFromResource:@"Config/client.sign"] ;
...
+ (void)install {
    [MSAppCoreInfo sharedMSAppCoreInfo];
}
...
+ (DataAppCacheDB *)getCacheDB {
    return [MSAppCoreInfo sharedMSAppCoreInfo].appCacheDB;
}

/** 获取全局应用设定数据库对象 */
+ (DataAppCoreDB *)getCoreDB {
    return [MSAppCoreInfo sharedMSAppCoreInfo].appCoreDB;
}

/** 获取全局缓存任务队列 */
+ (NSOperationQueue *)getCacheQueue {
    return [MSAppCoreInfo sharedMSAppCoreInfo].appCacheQueue;
}

/** 获取全局应用任务队列 */
+ (NSOperationQueue *)getCoreQueue {
    return [MSAppCoreInfo sharedMSAppCoreInfo].appCoreQueue;
}
...
- (void)setDeviceToken:(NSString *)deviceToken {
    if (deviceToken.length == 0) {
        ...
        [[MSAppCoreInfo getCoreDB] setStrValue:STORE_KEY_DEVICE_UUID dataKey:@"deviceToken" dataValue:deviceToken];
    }
...
+ (NSString *)deviceToken {
    NSString *dt = [MSAppCoreInfo sharedMSAppCoreInfo].deviceToken;
    if (dt.length == 0) {
        dt = [[MSAppCoreInfo getCoreDB] getStrValue:STORE_KEY_DEVICE_UUID dataKey:@"deviceToken"];
    }
    if (dt.length == 0) {
        return @"";
    }
...
/** 设备唯一标识 */
+ (NSString *)idfv {
    return [[MSAppCoreInfo sharedMSAppCoreInfo] idfvString];
}
...
- (NSString *)keychainIdString {
    ...
    if(identifier && identifier.length > 0) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7) {
            ...
            NSString *macaddress = [MSAppCoreInfo sharedMSAppCoreInfo].mac;
...
+(NSString *)sb_description {
    ...
    MSAppCoreInfo *info = [MSAppCoreInfo sharedMSAppCoreInfo];
    ...
    [dc appendFormat:@"电话运营商  =  %@\n", [MSAppCoreInfo telephonyNetworkInfo]];
    [dc appendFormat:@"屏幕参数  =  %@\n", [MSAppCoreInfo screenResolution]];
