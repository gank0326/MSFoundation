...
- (CGSize)sb_sizeWithFont:(UIFont *)font forWidth:(CGFloat)width lineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    return ajustedSize([self boundingRectWithSize:CGSizeMake(width, INT_MAX)
                                          options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
...
{
    NSMutableString *mStr = [[[NSMutableString alloc]initWithCapacity:self.length]autorelease];
    // 处理所有需要escape的字符
